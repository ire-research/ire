...
5. Load `workspace.json` if present (restores pane layout, tab metadata, and chat options). For each tab with a `historySessionUuid`, fetch its message history from the `chat_sessions` table (see [chat-agents.md — History persistence](chat-agents.md#multi-tab-chat)) — `workspace.json` no longer stores message arrays.
