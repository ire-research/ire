...
`type`/`status`/`scope`/`asserted_by`/`revision` are flat frontmatter scalars (the hand-rolled `frontmatter::parse` only handles scalars and flat string lists, not nested YAML) — `evidence[]` and `relations[]` from the claim schema are markdown sections in the body instead of structured frontmatter.

- `IreStore::write_claim(rel_path, content)` — atomic write, regenerate `claims/_index.md`. No CAS: the caller sends the full file each time, so a revision is a full overwrite, not a diff. No event emission yet — there is no claims UI panel to listen for one.
- `claims/_index.md` is auto-generated by `index::build_claims`: one bullet `- [id](./id.md) — status — <first body paragraph>` per file, sorted by filename. `status` defaults to `proposed` if the frontmatter omits it.
