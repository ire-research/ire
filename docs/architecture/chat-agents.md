...
pub trait AgentProvider: Send + Sync {
    ...
    fn persist_resume_id(&self, home_data_dir: &Path, session_uuid: &str, resume_id: &str,
                          meta: Option<SessionMeta<'_>>) -> Result<(), AgentError>;
                                                           // default: upsert (meta = Some, first message) or
                                                           // update (meta = None, every later turn) in db::models
}
...
**Resume-id persistence is a default `AgentProvider` method, not a free function each call site reimplements.** All four call sites — `commands/chat.rs` (CLI path, called from `StreamEvent::Init` handling), `opencode/turn.rs` (called synchronously once a session is created — OpenCode has no stream-derived id to hook), `experiments/wake.rs` (the wake-up turn, always `meta: None`), and `commands/resources.rs` (resource-ingestion turns, keyed by `tab_id` — see below) — call `agent.persist_resume_id(...)` at their own trigger point instead of each writing to `db::models` directly, which used to let the write drift out of sync across adapters. Failure is propagated, not swallowed: a dropped resume id would mean the next turn would silently start a new conversation while believing it resumed one, so all four call sites abort the turn (kill the subprocess, surface an error) rather than log-and-continue.

The resume id itself is not one of IRE's own identifiers — it's the underlying CLI's session/thread id (Claude Code's `session_id`, Codex's `thread_id`, OpenCode's session id), and it can rotate on every resume. That's why `persist_resume_id` runs on *every* turn's `Init`, not just the first: each call just overwrites the pointer passed back as `--resume` next time, to stay in sync with whatever id the CLI is running under right now. `session_uuid`/`model`/`provider` are IRE's own and are frozen after the first message (below) — only this CLI-side id changes turn to turn.

**`model` and `provider` are frozen once a session's first message is saved.** `db::models::verify_session_identity` is checked once per `chat_send`, before branching into the CLI or OpenCode path, and rejects a turn whose requested `(provider, model)` doesn't match what the session was created with; the same freeze is enforced in the UI by disabling `Composer`'s model picker once a tab's `messages` array is non-empty. `upsert_chat_resume_id`'s `chat_sessions` insert is `ON CONFLICT DO NOTHING` (was: update `provider`) and `insert_chat_session`'s upsert no longer touches `provider`/`model` on conflict, for the same reason — a session's identity is set once, at creation, and every later write (resume id, history save) can only touch fields that are allowed to change (label, message content, timestamps).
...
`commands/system::list_agent_models` is the IPC command exposing this: for Claude/Codex it calls `agent_provider::all()` (filtering out OpenCode) and builds one `ProviderCapabilities { provider, default_model, catalog }` per pair via the synchronous `ModelCatalog` path; for OpenCode it separately calls `OpenCodeRuntime::ensure_started` (starting the server if this is the first OpenCode use this session) then `OpenCodeClient::list_models` (`GET /provider`, flattened across every *connected* (authenticated) provider — `all` lists ~170 providers OpenCode merely knows the shape of, most needing an API key the user never set; filtering to `connected` matches what the `opencode models` CLI catalog always showed), building the same `ProviderCapabilities` shape by hand. Either path's `catalog: ModelCatalogStatus` is:
...
**Dynamic model catalog.** No `opencode models` CLI parsing. `OpenCodeClient::list_models` calls `GET /provider` and flattens every model across every provider in the response's `connected` list (`<providerID>/<modelID>` id, `Model.name` as label, `Model.variants` keys as `effort_levels`) — `all` is far broader (every provider OpenCode has a model definition for, ~170 of them, regardless of whether the user has an API key for it), so filtering to `connected` is what makes this match the old CLI catalog's behavior rather than regress it. See "Model catalog" above for how `list_agent_models` fetches this (it needs a live server, so it can't go through the synchronous `ModelCatalog` trait the other two providers use).
...
`chat_send` receives `session_uuid`, `tab_label`, and `started_at` from the frontend. It reads the resume id via `get_chat_resume_id(session_uuid, provider)` and passes `--resume <session_id>` for Claude, `codex exec resume <thread_id>` for Codex, or (for OpenCode) reuses the OpenCode session id directly — see [OpenCode Server Transport](#opencode-server-transport) above. On the first `Init` event of a turn (CLI transport) or immediately on session resolution (OpenCode transport) it calls `agent.persist_resume_id(...)`, creating the `chat_sessions`/`chat_resume_ids` rows if they don't exist yet (`meta: Some`) or just updating the resume id (`meta: None`) once they do. Because the row is keyed by `(session_uuid, provider)`, closing and reopening a workspace resumes the existing session for whichever provider the tab last used, and toggling provider keeps each provider's thread independently resumable.

`experiment.start` records the running turn's `tab_id`, `session_uuid`, `provider`, `model`, and optional `effort` (via `get_active_session`, which matches the first tab with *any* `RunningTurn` — `Process` or `OpenCode`, any provider can kick off an experiment) before spawning the detached command. The wake-up (`fire_wakeup`) resolves the resume id from `chat_resume_ids` by `session_uuid` + provider, and persists any new resume id the wake turn emits (`agent.persist_resume_id(..., None)` for the CLI path — the row is guaranteed to exist, an experiment always resumes an already-saved conversation; the same `opencode::turn::send` path as any other OpenCode turn otherwise). Resource-ingestion turns have no frontend history uuid, so they key their resume row by `tab_id`.
