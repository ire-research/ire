...
Resources are otherwise read with the built-in `Read` tool against `.ire/resources/` (and its `_index.md`); there is no `wiki.*` or resource-read MCP tool, and `experiment.list` is dropped (read a run's `.ire/experiments/<NNN>-<slug>/EXPERIMENT.md` instead).
