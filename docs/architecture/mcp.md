...
| `claim.write({ id, markdown })` | Write or revise a claim: atomically writes the full agent-supplied markdown to `claims/<slugified-id>.md` and regenerates `claims/_index.md`. No draft/approve step, no CAS — writing an existing id overwrites that file (a revision). Returns `dangling_references`/`hint` in the result if the claim's `depends_on`/`contradicts`/`supersedes` name ids with no matching file. Does not commit. |
