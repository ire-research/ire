...
pub fn tool_kind(normalized_name: &str) -> ToolKind {
    match normalized_name.to_ascii_lowercase().as_str() {
        ...
        "pulse.update" => ToolKind::PulseUpdate,
...
pub fn tool_title(kind: &ToolKind) -> &'static str {
    match kind {
        ...
        ToolKind::WikiRead => "Read wiki",
        ToolKind::WikiWrite => "Write wiki",
        ToolKind::WikiAppend => "Append wiki",
        ToolKind::WikiRename => "Rename wiki",
        ToolKind::MemoryWrite => "Write memory",
        ToolKind::PulseUpdate => "Update pulse",
