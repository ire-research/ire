...
// Resume ids so a chat can be re-attached to its underlying agent session after
// close/reopen. Stored per-provider so toggling claude<->codex keeps both threads
// resumable.
