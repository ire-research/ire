...
pub fn start_experiment(
    ...
    let data_dir = crate::workspace::init::home_data_dir(workspace_root)
    ...
    db::update_experiment_pid(&data_dir, &uuid, pid).ok();

    let _ = app.emit(
        "experiment-status",
        serde_json::json!({ "uuid": uuid, "status": "running" }),
    );
    if let Ok(Some(row)) = db::get_experiment(&data_dir, &uuid) {
    ...
    loop {
        ...
        match child.try_wait() {
            Ok(Some(status)) => {
                ...
                db::update_experiment_completed(&data_dir, &uuid, status_str, Some(exit_code)).ok();
            ...
            Err(e) => {
                ...
                db::update_experiment_completed(&data_dir, &uuid, "failed", Some(-1)).ok();
                let _ = app.emit(
                    "experiment-status",
