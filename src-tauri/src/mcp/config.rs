...
/// Write `mcp.json` into the workspace's home data dir to spawn the MCP server.
...
pub fn write_mcp_config(home_data_dir: &Path, workspace_root: &Path, socket_path: &Path) -> Result<()> {
    ...
    std::fs::write(home_data_dir.join("mcp.json"), json).context("write mcp.json")?;
    Ok(())
}
